    ...
    S: NonlinearSystem<T>,
{
    ...
    pub fn forecast(&mut self) {
        ...
        for j in 0..m {
            ...
            let propagated = self.system.state_transition(&member, self.control.as_deref(), self.dt);
    ...
    pub fn update(&mut self, observation: &[T]) -> KalmanResult<()> {
        ...
        for i in 0..n {
            for k in 0..m_obs {
                for j in 0..m_state {
                    ...
                    P_xh[i * m_obs + k] = P_xh[i * m_obs + k] + 
                        x_anomaly * h_anomaly / T::from(m_state - 1).unwrap();
                }
            }
        }
        ...
        for i in 0..n {
            for j in 0..m_obs {
                for k in 0..m_obs {
                    K[i * m_obs + j] = K[i * m_obs + j] + P_xh[i * m_obs + k] * P_hh_inv[k * m_obs + j];
                }
            }
        }
        ...
        for j in 0..m_state {
            ...
            for i in 0..n {
                for k in 0..m_obs {
                    self.ensemble[i * m_state + j] = self.ensemble[i * m_state + j] + 
                        K[i * m_obs + k] * innovation[k];
                }
            }
        }
