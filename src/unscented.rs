    ...
    S: NonlinearSystem<T>,
{
    ...
    pub fn new(
        ...
        // Compute lambda
        let lambda = params.alpha * params.alpha * (T::from(n).unwrap() + params.kappa) - T::from(n).unwrap();
        ...
        weights_cov[0] = lambda / (T::from(n).unwrap() + lambda) + (T::one() - params.alpha * params.alpha + params.beta);
    ...
    pub fn set_parameters(&mut self, params: UKFParameters<T>) {
        ...
        // Recompute lambda
        self.lambda = params.alpha * params.alpha * (T::from(n).unwrap() + params.kappa) - T::from(n).unwrap();
    ...
    pub fn predict(&mut self) -> KalmanResult<()> {
        ...
        for i in 0..num_sigma {
            ...
            let transformed = self.system.state_transition(&sigma_state, self.control.as_deref(), self.dt);
