...
use kalman_filter::{
    KalmanFilterBuilder, ExtendedKalmanFilter, UnscentedKalmanFilter, 
    information::InformationFilter, ParticleFilter,
    NonlinearSystem,
};
...
fn bench_matrix_operations(c: &mut Criterion) {
    ...
    for &dim in &dimensions {
        ...
        group.bench_with_input(
            BenchmarkId::new("matrix_inversion", dim),
            &dim,
            |b, _| {
                b.iter(|| {
...
fn bench_filter_variants(c: &mut Criterion) {
    ...
    group.bench_function("particle_filter_1000", |b| {
        b.iter(|| {
            ...
            black_box(pf.update(black_box(&measurement), |state, measurement| {
...
fn bench_particle_scaling(c: &mut Criterion) {
    ...
    for &count in &particle_counts {
        ...
        group.bench_with_input(
            BenchmarkId::new("particle_count", count),
            &count,
            |b, _| {
                b.iter(|| {
                    pf.predict(|state, dt| {
    ...
    for &dim in &dimensions {
        for &sparsity in &sparsity_levels {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        black_box(matrix_vector_multiply(black_box(&dense_matrix), black_box(&vec![1.0; dim]), dim));
                    });
                },
            );
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        black_box(sparse_matrix_vector_multiply(black_box(&sparse_matrix), black_box(&vec![1.0; dim]), dim));
                    });
                },
            );
        }
...
fn bench_memory_allocations(c: &mut Criterion) {
    ...
    for &dim in &dimensions {
        ...
        group.bench_with_input(
            BenchmarkId::new("state_clone", dim),
            &dim,
            |b, _| {
                b.iter(|| {
...
fn create_test_kalman_filter(state_dim: usize, measurement_dim: usize) -> kalman_filter::KalmanFilter<f64> {
...
fn create_test_kalman_filter_f32(state_dim: usize, measurement_dim: usize) -> kalman_filter::KalmanFilter<f32> {
    KalmanFilterBuilder::new(state_dim, measurement_dim)
...
impl TestNonlinearSystem {
    fn new(state_dim: usize, measurement_dim: usize) -> Self {
        Self { state_dim, measurement_dim }
    }
}
...
impl NonlinearSystem<f64> for TestNonlinearSystem {
    ...
    fn state_dim(&self) -> usize { self.state_dim }
    fn measurement_dim(&self) -> usize { self.measurement_dim }
}
